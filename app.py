...
# --- 2. Professional Bluish-Light CSS ---
st.markdown("""
    <style>
        /* Base Background: Premium Soft Light Blue */
        .stApp {
            background: linear-gradient(135deg, #E0F2FE 0%, #F8FAFC 100%);
        }
        
        /* Side Navigation: Deep Professional Blue */
        [data-testid="stSidebar"] {
            background: linear-gradient(180deg, #1E3A8A 0%, #1E1B4B 100%) !important;
            border-right: 1px solid rgba(0,0,0,0.05);
        }
        
        /* Sidebar Text: Soft White for readability against dark */
        [data-testid="stSidebar"] h1, [data-testid="stSidebar"] h2, [data-testid="stSidebar"] h3, 
        [data-testid="stSidebar"] label, [data-testid="stSidebar"] p, [data-testid="stSidebar"] span {
            color: #F0F9FF !important;
        }

        /* Main Workspace: Frosted White Glassmorphism */
        .main-glass-card {
        ...
        /* TYPOGRAPHY: STRICT BLACK FOR MAIN CONTENT */
        h1, h2, h3, p, label, .stMarkdown, b, strong, span {
            color: #000000 !important;
            font-family: 'Inter', sans-serif;
        }
        
        h1 { font-weight: 900 !important; letter-spacing: -1.5px; }
        ...
        /* Execute Button: High Visibility */
...
with st.sidebar:
    ...
    st.caption(f"Selected Tool: {st.session_state.active_gate}")
    p_cols = st.columns(2)
    ...
    st.markdown("---")
    if st.button("Reset Circuit Designer"):
        initialize_state(num_qubits, num_steps)
        st.rerun()
...
for i in range(num_steps):
    grid_cols[i+1].markdown(f"<center><b style='color:#475569'>{i}</b></center>", unsafe_allow_html=True)
...
if 'last_results' in st.session_state:
    ...
    st.markdown('<h2 style="text-align:center;">Quantum Phase Space (Bloch Spheres)</h2>', unsafe_allow_html=True)
